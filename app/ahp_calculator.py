...
class AHPCalculator:
    ...
    def calculate_weights(self) -> Dict[str, float]:
        ...
        self.weights = {
            self.criteria_names[i]: float(self.eigenvector[i]) for i in range(self.n)
        }
        return self.weights
    ...
    def get_results(self) -> Dict:
        ...
        return {
            "pairwise_matrix": (
                self.pairwise_matrix.tolist()
                if self.pairwise_matrix is not None
                else None
            ),
            "eigenvector": (
                self.eigenvector.tolist() if self.eigenvector is not None else None
            ),
            "lambda_max": (
                float(self.lambda_max) if self.lambda_max is not None else None
            ),
...
class FuzzyAHPCalculator:
    ...
    def compare_fuzzy_probability(
        self, m1: Tuple[float, float, float], m2: Tuple[float, float, float]
    ) -> float:
        l1, m1_val, u1 = m1
        l2, m2_val, u2 = m2
    ...
    def get_results(self) -> Dict:
        ...
        return {
            "fuzzy_pairwise_matrix": (
                self.fuzzy_pairwise_matrix.tolist()
                if self.fuzzy_pairwise_matrix is not None
                else None
            ),
