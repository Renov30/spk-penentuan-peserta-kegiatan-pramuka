...
def calculate_ahp_weights(
    event_id: int, pairwise_matrix: Optional[np.ndarray] = None
) -> Tuple[bool, str, Dict]:
    # Ambil kriteria
    criterias = (
...
def save_pairwise_matrix(
    event_id: int, criteria_ids: List[int], matrix: np.ndarray
) -> Tuple[bool, str]:
    try:
        # Hapus data lama
...
def calculate_spk(event_id: int, use_fuzzy_ahp: bool = True) -> Tuple[bool, str]:
    ...
    if total_bobot == 0:
        return (
            False,
            "Bobot kriteria belum dihitung. Silakan input matriks perbandingan berpasangan terlebih dahulu.",
        )
    ...
    for uid in participant_ids:
        ...
        for cid, weight in criteria_weights.items():
            ...
            avg_score = (
                ...
                .filter_by(id_users=uid, id_kriteria=cid)
                .scalar()
            )
        ...
        if has_score:
            ...
            final_scores.append({"user_id": uid, "score": defuzzified_score})
    ...
    try:
        ...
        if final_scores:
            ...
            HasilSeleksi.query.filter(
                HasilSeleksi.id_users.in_(user_ids), HasilSeleksi.event_id == None
